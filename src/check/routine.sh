...
GO "H1-4" 0 $ROUTINE_DEBUG $DAR -N -Q -t $xformed1 -A $catf_fly -aiesh -B $OPT
...
GO "H1-7" 0 $ROUTINE_DEBUG $DAR -N -Q -t $xformed2 -A $catf_fly -aiesh -B $OPT
if [ ! -z "$slicing" ] ; then
...
GO "H1-A" 0 $ROUTINE_DEBUG $DAR -N -Q -t $xformed3 -A $catf_fly -aiesh -B $OPT
