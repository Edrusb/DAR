...
namespace libdar
{
		     ...
		     const archive_options_read & options)
    {
		...
		local_path = new (nothrow) path(chem);
		if(local_path == NULL)
		    throw Ememory("archive::archive");
		exploitable = true;
	    }
	...
	NLS_SWAP_IN;
	    ...
	    parent = const_cast<directory *>(get_cat().get_contenu());
	    if(parent == NULL)
		throw SRC_BUG;
		...
		while(loop)
		{
		    ...
		    parent = dynamic_cast<directory *>(tmp_ptr);
		    if(parent == NULL)
	    ...
	    while(const_cast<const directory *>(parent)->read_children(const_cast<const nomme *&>(tmp_ptr)))
	    {
		if(tmp_ptr == NULL)
		    throw SRC_BUG;
	...
	local_path = new (nothrow) path(sauv_path); // [object member variable]
	if(local_path == NULL)
	    throw Ememory("archive::op_create_in");

	try
	{
	    path sauv_path_abs = sauv_path.is_relative() ? tools_relative2absolute_path(sauv_path, tools_getcwd()) : sauv_path;
		...
		ref_cat = new (nothrow) catalogue(dialog, 0, data_name);    // here this is a new object we will have to release at the end of this method
		if(ref_cat == NULL)
		    throw Ememory("op_create_in");
	...
	try
	{
	    ...
	    if(ref_cat1 != NULL && op == oper_isolate)
	    {
