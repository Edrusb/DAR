...
namespace libdar
{
    ...
    static void normalize_link_base(map<infinint, etoile *> & hard_link_base);
    ...
    {
	infinint max_val = 0;
	map<infinint, etoile *>::iterator it, ut;
	...
	while(search < num_etoile)
	{
		...
		{
		    if(max_val <= search)
			throw SRC_BUG; // num_etoile > 0, but we cannot find a etoile to move into the hole
		    ut = hard_link_base.find(max_val);
		    --max_val;
		}
		while(ut == hard_link_base.end());

		    // we can now renumber etoile tmp from max_val to search

		etoile *tmp = ut->second;
		if(tmp == NULL)
		    throw SRC_BUG;
