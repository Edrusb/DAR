...
namespace libdar
{
    ...
    const string LIBDAR_STACK_LABEL_ZOOM = "ZOOM";
	...
	try
	{
		...
		// *********** adding the zoom tronc layer before the compressor ***** //

	    tmp = new (pool) tronc(stack.top(), 0, false); // not limiting scope for now, will be changed on-fly later
	    if(tmp == NULL)
		throw Ememory("open_archive");
	    stack.push(tmp);
	    tmp = NULL;
	    stack.add_label(LIBDAR_STACK_LABEL_ZOOM);
